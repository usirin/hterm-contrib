...
import {
  emitKeys, getStringWidth, isFullWidthCodePoint, stripVTControlCharacters
} from './InternalReadline'
...
export default class Interface extends EventEmitter {
  constructor(input, output, completer, terminal) {
    ...
    if (!this.terminal) {
      ...
      Interface.emitKeypressEvents(input, this);
...
Interface.prototype._refreshLine = function() {
  ...
  if (prevRows > 0) {
    Interface.moveCursor(this.output, 0, -prevRows);
  }

  // Cursor to left edge.
  Interface.cursorTo(this.output, 0);
  // erase data
  Interface.clearScreenDown(this.output);
  ...
  Interface.cursorTo(this.output, cursorPos.cols);

  var diff = lineRows - cursorPos.rows;
  if (diff > 0) {
    Interface.moveCursor(this.output, 0, -diff);
  }
...
Interface.prototype._moveCursor = function(dx) {
  ...
  if (oldPos.rows === newPos.rows) {
    ...
    Interface.moveCursor(this.output, diffWidth, 0);
  ...
  if (key.ctrl && key.shift) {
    ...
    switch (key.name) {
      ...
      case 'l': // clear the whole screen
        Interface.cursorTo(this.output, 0, 0);
        Interface.clearScreenDown(this.output);
        this._refreshLine();
        break;
...
Interface.emitKeypressEvents = function emitKeypressEvents(stream, iface) {
...
Interface.cursorTo = function cursorTo(stream, x, y) {
...
Interface.moveCursor = function moveCursor(stream, dx, dy) {
  if (stream === null || stream === undefined)
    return;
...
Interface.clearLine = function clearLine(stream, dir) {
  if (stream === null || stream === undefined)
    return;
...
Interface.clearScreenDown = function clearScreenDown(stream) {
  if (stream === null || stream === undefined)
    return;
